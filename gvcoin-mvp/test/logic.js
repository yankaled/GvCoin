...
const namespace = 'gvcoin';
const assetType = 'GvCoin';
...
describe('#' + namespace, () => {
    ...
    describe('ChangeGvCoinOwner()', () => {
        it('should change the value property of ' + assetType + ' to newValue', () => {
            ...
            const ChangeGvCoinOwner = factory.newTransaction(namespace, 'ChangeGvCoinOwner');
            ChangeGvCoinOwner.relatedAsset = factory.newRelationship(namespace, assetType, asset.$identifier);
            ChangeGvCoinOwner.newValue = 'new-value';
            ...
            return businessNetworkConnection.getAssetRegistry(namespace + '.' + assetType).then(registry => {
                ...
                return businessNetworkConnection.submitTransaction(ChangeGvCoinOwner);
            }).then(registry => {
                // Get the asset
                return assetRegistry.get(asset.$identifier);
            }).then(newAsset => {
                // Assert that the asset has the new value property
                newAsset.value.should.equal(ChangeGvCoinOwner.newValue);
            });
        });
    });
